...
# 🍲 飢荒神級砂鍋食譜模擬器 (Don't Starve Crock Pot Recipe Simulator)

本專案是一個專為《Don't Starve Together》(飢荒) 玩家設計的 Crock Pot 砂鍋食譜模擬器與資料庫檢索系統。玩家可以透過直覺的介面勾選現有食材、篩選食譜，或是直接在模擬器中放入食材進行「虛擬烹飪」，計算出最終的料理結果與其生命值、飢餓值和精神值恢復數據。
